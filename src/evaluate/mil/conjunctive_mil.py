...
class ConjunctiveClassifier(nn.Module):
    def __init__(
        self,
        input_dim: int,
        output_dim: int,
    ...
    def forward(self, features, attention, bag_sizes):
        ...
        for idx, size in enumerate(bag_sizes.tolist()):
            ...
            bag_features = self.model(features[start:end])
            bag_attention = attention[idx].unsqueeze(-1)
            ins_logit = bag_features * bag_attention
            bag_logit = self.conjunctive_function.pool(ins_logit, dim=0, keepdim=True)
